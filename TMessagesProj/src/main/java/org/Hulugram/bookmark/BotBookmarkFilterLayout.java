...
public class BotBookmarkFilterLayout extends FrameLayout {

    private HashMap<Long, TLRPC.User> userHashMap = new HashMap<>();
    ...
    public BotBookmarkFilterLayout(@NonNull BaseFragment fragment, BookmarkViewPager.MediaCountLoader preloaderDelegate) {
        ...
        listView.setOnItemClickListener(BotBookmarkFilterLayout.this::onItemClick);
        listView.setOnItemLongClickListener((view, position) -> {
            BotBookmarkFilterLayout.this.onItemLongClick(view,position);
            return true;
        });
    ...
    private void updateEmptyView() {
        if (loading) {
            ...
            emptyTitleTextView.setText(LocaleController.getString("HuluNoBookmarkUsers", R.string.HuluNoBookmarkUsers));
            emptySubtitleTextView.setText(LocaleController.getString("HuluNoBookmarkUserInfo", R.string.HuluNoBookmarkUserInfo));
    ...
    private void onItemClick(View view,int pos){
        ...
        TLRPC.User chat = userHashMap.get(bookmarks.get(pos).dialog_id);
        if (chat == null) {
            return;
        }
        ...
        bundle.putLong("user_id", chat.id);
    ...
    private boolean onItemLongClick( View view,int pos) {
        ...
        TLRPC.User chat = userHashMap.get(bookmark.dialog_id);
        if (chat == null) {
            return false;
        }
        if (!bookmarkUiCallback.actionModeShowing()) {
    ...
    public void update() {
        ...
        parentFragment.getMessagesStorage().getStorageQueue().postRunnable(() -> {
            ...
            ArrayList<Long> userToLoad = new ArrayList<>();
            ArrayList<TLRPC.User> userInternal = new ArrayList<>();
            HashMap<Long, TLRPC.User> chatHashMapInternal = new HashMap<>();
            try{
                ...
                if(!userToLoad.isEmpty()){
                    parentFragment.getMessagesStorage().getUsersInternal(TextUtils.join(",",userToLoad),userInternal);
                }
                for(int a = 0; a < userInternal.size();a++){
                    chatHashMapInternal.put(userInternal.get(a).id,userInternal.get(a));
                }

                AndroidUtilities.runOnUIThread(() -> {
                    loading = false;
                    userHashMap = chatHashMapInternal;
                    bookmarks = bookmarksInternal;
                    if(listAdapter != null){
                        listAdapter.notifyDataSetChanged();
                    }
                    if(preloaderDelegate != null){
                        preloaderDelegate.getLastMediaCount()[BookmarkViewPager.BOT_TYPE] = bookmarks.size();
    ...
    private class ListAdapter extends RecyclerListView.SelectionAdapter{
        ...
        public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
            ...
            if(chat != null){
                ...
                userCell.setObject(chat, UserObject.getUserName(chat),countString,true);
                boolean animated = userCell.getObject() != null && userCell.getObject() instanceof TLRPC.User && ((TLRPC.User)userCell.getObject()).id == chat.id;
        ...
        for (int j = 0; j < bookmarks.size(); j++) {
            ...
            for (int i = 0; i < markAsDeletedMessages.size(); i++) {
                if (messageObject.dialog_id == markAsDeletedMessages.get(i)) {
                    changed = true;
                    bookmarks.remove(j);
                    userHashMap.remove(messageObject.dialog_id);
                    j--;
                }
            }
        }
