...
public class DilogCacheBottomSheet extends BottomSheetWithRecyclerListView {
    ...
    CacheFragment.DialogFileEntities entities;
    private final Delegate cacheDelegate;
    private CacheFragment.ClearCacheButton button;
    ...
    public DilogCacheBottomSheet(BaseFragment baseFragment, CacheFragment.DialogFileEntities entities, CacheModel cacheModel, Delegate delegate) {
        ...
        for (int a = 0; a < 7; a++) {
            ...
            if (entities != null) {
                CacheFragment.FileEntities fileEntities = entities.entitiesByType.get(a);
...
//                    }
        ...
        cachedMediaLayout.setDelegate(new CachedMediaLayout.Delegate() {
            @Override
            public void onItemSelected(CacheFragment.DialogFileEntities entities, CacheModel.FileInfo fileInfo, boolean longPress) {
                if (fileInfo != null) {
    ...
    private void syncCheckBoxes() {
        if (checkBoxes[CacheFragment.TYPE_PHOTOS] != null) {
            checkBoxes[CacheFragment.TYPE_PHOTOS].setChecked(clearViewData[CacheFragment.TYPE_PHOTOS].clear = cacheModel.allPhotosSelected, true);
        }
        if (checkBoxes[CacheFragment.TYPE_VIDEOS] != null) {
            checkBoxes[CacheFragment.TYPE_VIDEOS].setChecked(clearViewData[CacheFragment.TYPE_VIDEOS].clear = cacheModel.allVideosSelected, true);
        }
        if (checkBoxes[CacheFragment.TYPE_DOCUMENTS] != null) {
            checkBoxes[CacheFragment.TYPE_DOCUMENTS].setChecked(clearViewData[CacheFragment.TYPE_DOCUMENTS].clear = cacheModel.allDocumentsSelected, true);
        }
        if (checkBoxes[CacheFragment.TYPE_MUSIC] != null) {
            checkBoxes[CacheFragment.TYPE_MUSIC].setChecked(clearViewData[CacheFragment.TYPE_MUSIC].clear = cacheModel.allMusicSelected, true);
        }
        if (checkBoxes[CacheFragment.TYPE_VOICE] != null) {
            checkBoxes[CacheFragment.TYPE_VOICE].setChecked(clearViewData[CacheFragment.TYPE_VOICE].clear = cacheModel.allVoiceSelected, true);
        }
    }
    ...
    private void createButton() {
        button = new CacheFragment.ClearCacheButton(getContext());
    ...
    public interface Delegate {
        ...
        void cleanupDialogFiles(CacheFragment.DialogFileEntities entities, StorageDiagramView.ClearViewData[] clearViewData, CacheModel cacheModel);
    }
}
