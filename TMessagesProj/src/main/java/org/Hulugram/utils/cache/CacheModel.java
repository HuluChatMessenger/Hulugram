...
public class CacheModel {
    ...
    public ArrayList<CacheFragment.DialogFileEntities> entities = new ArrayList<>();
    private final LongSparseArray<CacheFragment.DialogFileEntities> entitiesByDialogId = new LongSparseArray<>();
    ...
    public void add(int addToType, FileInfo fileInfo) {
        if (addToType == CacheFragment.TYPE_PHOTOS) {
            media.add(fileInfo);
        } else if (addToType == CacheFragment.TYPE_VIDEOS) {
            media.add(fileInfo);
        } else if (addToType == CacheFragment.TYPE_DOCUMENTS) {
            documents.add(fileInfo);
        } else if (addToType == CacheFragment.TYPE_MUSIC) {
            music.add(fileInfo);
        } else if (addToType == CacheFragment.TYPE_VOICE) {
            voice.add(fileInfo);
        }
    }

    private void remove(int type, FileInfo fileInfo) {
        if (type == CacheFragment.TYPE_PHOTOS) {
            media.remove(fileInfo);
        } else if (type == CacheFragment.TYPE_VIDEOS) {
            media.remove(fileInfo);
        } else if (type == CacheFragment.TYPE_DOCUMENTS) {
            documents.remove(fileInfo);
        } else if (type == CacheFragment.TYPE_MUSIC) {
            music.remove(fileInfo);
        } else if (type == CacheFragment.TYPE_VOICE) {
            voice.remove(fileInfo);
        }
    }

    public void remove(CacheFragment.DialogFileEntities dialogEntities) {
    ...
    public void toggleSelect(CacheFragment.DialogFileEntities entities) {
        if (!selectedDialogs.contains(entities.dialogId)) {
            for (int i = 0; i < entities.entitiesByType.size(); i++) {
                CacheFragment.FileEntities entriesType = entities.entitiesByType.valueAt(i);
    ...
    private void checkSelectedDialogs() {
        ...
        for (Long dialogId : dialogIdsTmp) {
            CacheFragment.DialogFileEntities entity = entitiesByDialogId.get(dialogId);
            if (entity != null) {
                for (int i = 0; i < entity.entitiesByType.size(); i++) {
                    CacheFragment.FileEntities entriesType = entity.entitiesByType.valueAt(i);
                    for (FileInfo fileInfo : entriesType.files) {
                        if (!selectedFiles.contains(fileInfo)) {
                            continue lookup;
                        }
    ...
    public void setEntities(ArrayList<CacheFragment.DialogFileEntities> entities) {
        this.entities = entities;
        entitiesByDialogId.clear();
        for (CacheFragment.DialogFileEntities entity : entities) {
    ...
    public CacheFragment.DialogFileEntities removeSelectedFiles() {
        CacheFragment.DialogFileEntities entities = new CacheFragment.DialogFileEntities(0);
        for (FileInfo fileInfo : selectedFiles) {
            entities.addFile(fileInfo, fileInfo.type);
            CacheFragment.DialogFileEntities entitiesFromRemove = entitiesByDialogId.get(fileInfo.dialogId);
    ...
    public void selectAllFiles() {
        for (int i = 0; i < media.size(); i++) {
            ...
            if (media.get(i).type == CacheFragment.TYPE_PHOTOS) {
    ...
    private void incSize(FileInfo fileInfo, boolean inc) {
        ...
        if (fileInfo.type == CacheFragment.TYPE_PHOTOS) {
            photosSelectedSize += size;
        } else if (fileInfo.type == CacheFragment.TYPE_VIDEOS) {
            videosSelectedSize += size;
        } else if (fileInfo.type == CacheFragment.TYPE_DOCUMENTS) {
            documentsSelectedSize += size;
        } else if (fileInfo.type == CacheFragment.TYPE_MUSIC) {
            musicSelectedSize += size;
        } else if (fileInfo.type == CacheFragment.TYPE_VOICE) {
